...
import {
  ...
  routeUnlockMet, routeFacilityUnlockMet, routeAtCell, nextArrivalText, PARKING_GUESTS_PER_SLOT,
} from '../entry.ts';
...
test('진입점 좌표 — 서(0,15) 버스 · 서(0,11) 올레 · 동(29,15) 렌터카 (trim: 경로 3종)', () => {
  ...
  expect(ROUTE_IDS).toEqual(['bus', 'parking', 'olle']);
});
...
test('시작 상태: 정류장만 열려 있고 나머지는 잠김·미연결', () => {
  ...
  expect(eps.find((e) => e.route === 'olle')).toMatchObject({ unlocked: false, connected: false, active: false });
...
test('경로 태그 가중치: 주차장은 가족 ×2·커플 ×1.6, 올레는 혼자 ×2', () => {
...
test('해금: 주차장은 처음부터(fun P0), 올레는 parcel4 — dailyRoutes가 시설·경로를 연다', () => {
  const s = cafe();
  ...
  dailyRoutes(s);
  for (const id of ['parking_lot', 'olle_sign']) expect(s.unlocked.objects, id).toContain(id);
...
test('목표 조건 routeGuests·routeUnlocked·facility', () => {
  ...
  expect(conditionProgress(s, { type: 'routeUnlocked', route: 'olle' }).cur).toBe(0);
  routeState(s, 'olle').unlocked = true;
  expect(conditionProgress(s, { type: 'routeUnlocked', route: 'olle' }).cur).toBe(1);
  ...
  expect(goalConditionText({ type: 'routeUnlocked', route: 'olle' })).toBe('올레길 열기');
...
test('fun P0 땅을 사면 경로가 열린다: 서쪽 밭담 골짜기 → 올레 표식(3,11)+진입점 올렛길 무료, 장면 창 한 줄', () => {
  const s = cafe();
  s.money = 100_000_000;
