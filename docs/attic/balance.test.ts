 ...
 *  야외 중심 개편 뒤 상한을 3,000 → 3,400으로 넓혔다. **까닭은 겨울이 아니라 땅이다.**
 *  증축 Lv2~4·2층·별관을 지우면서 봇이 쓰던 큰 지출이 사라져 3년차 말 자금이 ₩2.4억까지 튀었고,
 *  그 돈의 새 출구를 필지 구매로 옮겼다(BOT_PARCELS_PER_MONTH 1 → 2). 땅이 넓어진 만큼 시설이 늘고
 *  손님이 따라 는다 — 즉 연평균 상승은 「유일한 확장 축이 필지가 됐다」의 직접 결과다.
 *  (겨울 자체는 오히려 내려갔다: 3년차 겨울/나머지 비율 83% → 70%. 예전 83%는 봇이 실내 좌석을 14~20석
 *   깔아 얻던 겨울 이득이었고, 지금 70%는 설계된 비수기 곱 12~2월 ×0.7 그대로다. preferIndoor를 지운 이득은
 *   봇이 아니라 **사람** 쪽에 있다 — 실내 좌석 한 개만 있어도 야외 좌석 전부가 겨울 내내 꺼지던 것이 없어졌다.)
 *  실측 seed 1~3: 2,335 · 2,652 · 3,210. */
