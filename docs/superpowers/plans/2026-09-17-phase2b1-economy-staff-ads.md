...
### 사용자 피드백 반영 (구현 중 변경)
- 채취꾼 역할 삭제(`gather` 관련 코드·데이터·테스트 제거). 재료는 심은 작물에서만.
- `MenuDef.requires?: { role?: RoleId; skill?: string }` — 배치된 직원(기력>0)이 조건을 만족해야 메뉴 available. 시작 메뉴 중 라떼·핫초코=바리스타, 디저트·식사=요리사.
- `HOUR_MS = 2000`, 손님 착석 `SEAT_MS = 6000`(게임 ms). 손님이 너무 빨리 떠난다는 피드백.
- 맵 확장(필지 구매)은 2B-3 Task 4 — 2B-1 머지 직후 바로 착수.
