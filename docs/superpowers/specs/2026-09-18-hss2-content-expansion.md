...
## 7. 손으로 하는 튜토리얼·도전 과제·보상 연출 (트랙 B 확장)

사용자 피드백: "처음 길 잇고 테이블 설치하고 메뉴 넣고 이런 거 다 **직접 해 보는** 튜토리얼. 성장 욕구·도전 욕구를 확실히, 전략 요소에 집중, 보상을 확실히."

### 7.1 시작 상태 변경 (v3 §5 덮어씀)
본관(카운터·주방)과 정낭·정류장만. 길 없음, 좌석 없음, 메뉴 슬롯 비어 있음, 자금 500만, 직원 후보 2명 대기. 손님은 좌석·길·메뉴가 갖춰질 때까지 오지 않는다(정류장에 버스만 선다).

### 7.2 손으로 하는 튜토리얼 9단계 (`tutorial.json` 교체)
각 단계 = 할망 대화(2~3줄) → **하이라이트**(하단 버튼 글로우 / 맵 칸 글로우 / 창 안 버튼 글로우, `data-tut` 속성) → 플레이어가 직접 함 → 즉시 **보상 상자** 연출(§7.4) → 다음 단계. 건너뛰기는 첫 단계에만 있고, 건너뛰면 §7.1 대신 기존 완성 시작 상태로 채워 준다.

| # | 할 일 (조건) | 하이라이트 | 보상 | 대사 요지 |
|---|---|---|---|---|
| 1 | 정낭에서 문까지 올렛길 잇기 (`hasReachableSeat`가 아니라 `pathConnected(정낭, 문)`) | 짓기 버튼 → 길·담 탭 → 정낭·문 칸 글로우 | ₩30만 | "손님은 길로 온다. 정낭에서 문까지 이어 보라" |
| 2 | 테이블 1개를 **전망 2 이상 자리**에 놓기 (`site.view ≥ 2`) | 짓기 → 쉼 → 바닷가 방향 칸 3개 글로우 + 고스트 배지 설명 | ₩30만 + 「입지 보기」 해금 | "같은 테이블도 자리가 다르다. 바다 보이는 데 놓아 보라. 배지 숫자를 보라" |
| 3 | 메뉴판에 아메리카노·감귤주스 올리기 | 카페 버튼 → 메뉴판 → 올리기 버튼 글로우 | ₩20만 | "메뉴가 있어야 팔지" |
| 4 | 첫 손님 결제 | 정류장 글로우(버스) | 응모권 1 + 첫 뽑기 유도 | "왔다! 뭘 시키나 보라" |
| 5 | 직원 1명 채용 | 사람 → 후보 → 채용 글로우 | ₩30만 | "혼자는 못 한다" |
| 6 | 돌담 1개를 테이블 북서쪽에 놓기 (`site.wind` 감소 확인) | 짓기 → 담 → 테이블 북서 칸 글로우 | ₩30만 + 콤보 도감 해금 | "겨울 북서풍은 돌담이 막는다. 붙여 놓으면 **콤보**도 난다" |
| 7 | 홍보 1회 | 카페 → 홍보 | 마일리지 30 | "동네에 알려야지" |
| 8 | 목표 줄 탭해서 목표 창 보기 + 도전 과제 1개 수락 | 목표 줄 글로우 | ₩50만 | "이제부턴 저 줄이 네 길잡이다. 도전은 골라서 받으라" |
| 9 | 끝 (첫 월말 결산 닫기) | — | 명소 지도 해금 | "장부를 보며 키워 보라. 3월엔 가이드북이 나온다" |

### 7.3 목표 3레인 (정체 방지·도전 욕구)
- **메인 목표**(§3.5의 108개, 순차) + **도전 과제 2슬롯**(플레이어가 목록 6개 중 골라 수락, 기한 있음) + **월간 과제**(매월 1일 자동 1개, 그달 안).
- 도전 과제 풀 40개(`challenges.json`): `{ id, title, condition, days, reward, tier: 1~5, requires?: goalIndex }`. 예: "이번 주 안에 좌석 4개 → 파라솔 테이블 Lv2 해금", "30일 안에 자금 +300만 → 응모권 5", "전망 3 이상 좌석 5개 → 「바다 명당」 칭호 + 요금 +5%", "콤보 3개 완성 → 특수 아이템", "한 달 적자 없이 → 마일리지 50", "라이벌 대결 승리 → 시설 해금", "직원 연수 1회", "명소 Lv2 달성". 실패하면 페널티 없이 다시 고를 수 있으나 tier 높은 과제는 실패 시 3개월 잠김.
- 월간 과제는 난이도 자동(현재 손님 수의 1.3배 등). 보상은 응모권·마일리지.
- 상태: `state.challenges: { active: {id, startDay, endDay, progress}[], done: string[], failed: {id, until}[] }`, `state.monthly: {...}`. 목표 줄 아래에 도전 과제 진행 1줄(현재 가장 급한 것)이 추가되고(GoalBar에 두 번째 줄, 총 셸 높이 +20px), 목표 창에 [메인/도전/월간] 탭.
- 메인 목표 중 순위·★ 같은 외부 심사 의존 목표는 **"n개월 안"이 아니라 조건 충족 시** 자동 통과하되, 그 동안 뒤 목표가 막히지 않게 **메인도 2개 동시 진행**(현재 index와 index+1을 둘 다 표시·판정).

### 7.4 보상 상자 연출 (`RewardPopup.tsx`)
목표·도전·튜토리얼 보상은 전부 이 팝업: 갈색 상자가 흔들리다 열리며 보상 아이콘+수치가 하나씩 튀어나옴(0.3초 간격, `fanfare`/`coin` 효과음), 해금이면 시설/메뉴 스프라이트 + "새로 열렸다!" 배지, 탭하면 닫힘. 대화창(축하 대사)은 이 팝업 **뒤에** 이어서 뜬다. 보상은 `applyRewards()` 한 곳에서 지급하고 `state.alerts`에 `{type:'reward', items:[...]}`를 남겨 UI가 띄운다.

### 7.5 전략 목표 예시 (메인 108 안에 섞기)
전망 좌석 n개, 바람 0인 좌석 n개, 콤보 n개, 명당 1개, 시설 Lv2 n개, 청결 90 이상 유지 30일, 직원 특기 보유 n명, 명소 Lv3, 재료 자급률 50%(농원), 라이벌 이기기, 가이드북 항목 중 "경관" 1위 등 — 조건 타입은 §3.5의 14종 + `siteSeats(view≥n, count)`, `combos(n)`, `spotEffects(n)`, `upgraded(level, n)`, `clean(avg, days)`, `skills(n)`, `selfSupply(pct)`.
