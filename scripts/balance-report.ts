...
for (const seed of seeds.length ? seeds : [1, 2, 3]) {
  ...
  for (const r of rows) console.log(`  ${r.year}년 ${String(r.month).padStart(2)}월 net ${String(Math.round(r.net / 10000)).padStart(6)}만 money ${String(Math.round(r.money / 10000)).padStart(6)}만 guests ${String(r.guests).padStart(4)} staff ${r.staff} research ${String(r.research).padStart(4)} mileage ${r.tickets} rank ${r.rank} star ${r.star} custom ${r.customMenus}`);
