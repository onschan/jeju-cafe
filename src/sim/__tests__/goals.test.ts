...
describe('goals.json 데이터', () => {
  ...
  it('신설 조건 판정: 월 매출·Lv 직원·콤보·명소·손님 타입·가이드북·흑자 달·아이템·유니폼·특기·좌석·이달 손님/매출', () => {
    ...
    s.guidebooks['gb_kind_cafe'] = { unlocked: true, lastRank: 4, best: 4 };
...
describe('목표 체인 진행', () => {
  ...
  it('applyRewards 한 곳: 보상 상자 알림 하나에 아이템이 다 담기고, 대출 중이면 돈 50%·응모권/마일리지 절반(내림)', () => {
    ...
    (s as unknown as { loan: { remaining: number } }).loan = { remaining: 3_000_000 };
  ...
  it('신설 보상: 아이템·씨앗·손님 해금·가이드북 해금·칭호·요금 보너스·없는 직종 슬롯', () => {
    ...
    expect((s.slots as Record<string, number>)['clean']).toBe(1);
    expect(s.unlocked.roles).not.toContain('clean'); // 직종 정의가 생기면(x-staff) 열린다
