...
describe('데이터: 요청 30개', () => {
  test('id 고유·문구 ≤22자·want는 테마/시설/메뉴/태그 중 하나 이상·힌트가 있다', () => {
    ...
    for (const r of REQUESTS) {
      ...
      // 테마·태그 요청은 테마 데이터가 없는 동안(통합 전) 시설로 판정할 수 있어야 한다
...
describe('요청', () => {
  ...
  test('테마 요청: 테마가 완성돼 있으면 테마로, 아니면 대체 시설로 판정; tagCorner(photo)는 테마 태그로', () => {
    const s = cafe();
    ...
    expect(isRequestMet(t, def)).toBe(true); // 테마 우선
