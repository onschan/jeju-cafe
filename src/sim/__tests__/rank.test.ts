...
function richState(): GameState {
  const s = bareState(1);
  s.lastMonthCard = { income: 400_000, guests: 100, month: 3, year: 1, costs: emptyMonthCosts(), net: 400_000, ...emptyMonthHarvest(), topMenu: null, deficitStreak: 0, loanTaken: 0, loanBalance: 0, rivalLossPct: 0, guestsLeft: 0 };
...
test('심사 점수는 모두 0~100이고 같은 상태면 같은 값(결정적), 직원·좌석이 생기면 미소·경관이 오른다', () => {
  ...
  expect(guidebookScore(s, guidebookDef('gb_kind_cafe'), b)).toBe(Math.round(b.smile * 0.5 + b.menu * 0.1 + b.rest * 0.1 + b.clean * 0.2 + b.price * 0.1));
