...
import {
  ...
  RUSH_WEEKDAY, BATTLE_FROM_YEAR, BATTLE_NOTICE_DAYS, BATTLE_SURRENDER_STREAK, BATTLE_SURRENDER_GUESTS,
...
describe('일정', () => {
  test('러시는 토요일(day % 7 === 6), 대항전은 그달 마지막 러시 날', () => {
    expect(RUSH_WEEKDAY).toBe(6);
    expect(battleDay() % 7).toBe(RUSH_WEEKDAY);
