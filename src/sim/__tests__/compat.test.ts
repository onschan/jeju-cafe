...
test('상성 거리: 체비쇼프 2칸이면 발동, 3칸이면 아니다', () => {
  ...
  const t = placeObject(s, 'table_out', 6, 4);
  expect(activeCombos(s, t.id)).toEqual([]);
  const tree = placeObject(s, 'tangerine_tree', 8, 2); // dx 2, dy 2
  ...
  const t2 = placeObject(s2, 'table_out', 6, 4);
  placeObject(s2, 'tangerine_tree', 9, 4);
...
test('상성 거리: 여러 칸 오브젝트는 가장 가까운 발자국 칸으로 잰다', () => {
  const s = createInitialState(1); // 폐창고 (3,1) 3×2 → x 3..5, y 1..2
  const near = placeObject(s, 'table_out', 7, 4); // (5,2)에서 dx 2, dy 2
  const far = placeObject(s, 'table_out', 8, 4);  // dx 3
...
test('bCount: 필요 개수만큼 B가 있어야 발동', () => {
  ...
  const s = createInitialState(1);
  const t = placeObject(s, 'table_out', 6, 4);
  placeObject(s, 'tangerine_tree', 7, 4);
  expect(activeCombos(s, t.id, [two])).toEqual([]);
  placeObject(s, 'tangerine_tree', 5, 4);
...
test('히든 상성은 처음 발동할 때 한 번만 도감에 오르고 알림이 난다', () => {
  ...
  placeObject(s, 'table_out', 6, 3);
  discoverCombos(s, [hidden], []);
  expect(s.codex.combos).toEqual(['cb_hidden']);
  expect(s.notices.length).toBe(n0 + 1);
  // 보통 상성은 조용히 도감에만
  const s2 = createInitialState(1);
  placeObject(s2, 'table_out', 7, 4);
...
test('place 액션이 상성 발견을 돌린다', () => {
  const s = createInitialState(1);
  apply(s, { type: 'place', objectType: 'table_out', x: 6, y: 4 });
  expect(s.codex.combos).toEqual([]);
  apply(s, { type: 'place', objectType: 'tangerine_tree', x: 7, y: 4 });
...
test('세트: 반경 3 안에 필요 시설이 1배·2배·3배면 레벨 1·2·3', () => {
  ...
  const t = placeObject(s, 'table_out', 6, 4);
  placeObject(s, 'tangerine_tree', 7, 4);
  placeObject(s, 'stonewall', 8, 4);
  expect(setLevels(s, t.id, [EMO])).toEqual([]); // 테이블 1, 나무 1
  placeObject(s, 'table_out', 6, 5);
  placeObject(s, 'tangerine_tree', 7, 5);
  expect(setLevels(s, t.id, [EMO])[0]).toMatchObject({ id: EMO.id, level: 1, mult: 1.1 });
  placeObject(s, 'table_out', 8, 5); placeObject(s, 'table_out', 9, 5);
  placeObject(s, 'tangerine_tree', 9, 4); placeObject(s, 'tangerine_tree', 9, 3);
  expect(setLevels(s, t.id, [EMO])[0]!.level).toBe(1); // 돌담 1개 → 아직 1
  placeObject(s, 'stonewall', 8, 3);
  expect(setLevels(s, t.id, [EMO])[0]).toMatchObject({ level: 2, mult: 1.2 });
  placeObject(s, 'table_out', 5, 3); placeObject(s, 'table_out', 5, 5);
  placeObject(s, 'tangerine_tree', 5, 4); placeObject(s, 'tangerine_tree', 4, 4);
  placeObject(s, 'stonewall', 4, 3);
  expect(setLevels(s, t.id, [EMO])[0]).toMatchObject({ level: 3, mult: 1.35 });
  // 반경 밖(4칸)의 것은 안 센다
  const s2 = createInitialState(1);
  const t2 = placeObject(s2, 'table_out', 2, 4);
  placeObject(s2, 'table_out', 6, 4); placeObject(s2, 'tangerine_tree', 6, 5); placeObject(s2, 'tangerine_tree', 6, 3); placeObject(s2, 'stonewall', 6, 6);
...
test('세트 배수는 대상 태그 손님에게만 인기를 곱한다', () => {
  ...
  const t = placeObject(s, 'table_out', 6, 4);
  placeObject(s, 'table_out', 6, 5);
  placeObject(s, 'tangerine_tree', 7, 4); placeObject(s, 'tangerine_tree', 7, 5);
  placeObject(s, 'stonewall', 8, 4);
...
test('좌석 요금 배수: 귤밭 뷰(+5%)가 메뉴 가격에 곱해진다', () => {
  const s = createInitialState(1);
  placeObject(s, 'table_out', 4, 5); // 정낭 (4,6) 위
  placeObject(s, 'tangerine_tree', 5, 5);
