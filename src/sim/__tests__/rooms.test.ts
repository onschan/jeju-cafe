...
describe('본관 증축 (§8.1)', () => {
  test('Lv1 3×2 → Lv2 4×3: 비용 300만·7일·영업 정지, 원점 고정·남동으로 자람, 발자국 안 올렛길은 철거·환불, 기존 가구는 그대로', () => {
    ...
    placeObject(s, 'table_in', X(4), Y(1));
    ...
    expect(freeFloorCells(s, m).length).toBe(12 - 1 - 1); // 문·테이블
    expect(objectAt(s, X(4), Y(1))?.type).toBe('table_in'); // 가구 유지
  ...
  test('실내 칸 입지: 바람 0·그늘 2. 실내 가구는 방 안에만 (밖이면 "실내 가구는 건물 안에만 놓아요")', () => {
    const s = cafe();
    const site = siteOf(s, X(4), Y(1));
    expect(site.wind).toBe(0); expect(site.shade).toBe(2);
    expect(canPlace(s, 'sofa_seat', X(0), Y(0)).reason).toBe('실내 가구는 건물 안에만 놓아요');
    expect(canPlace(s, 'sofa_seat', X(4), Y(1)).ok).toBe(true);
    expect(canPlace(s, 'kids_corner', X(4), Y(1)).ok).toBe(true); // 2×2: (4,1)-(5,2) 전부 바닥·문(3,2) 아님
    expect(canPlace(s, 'kids_corner', X(3), Y(1)).reason).toBe('문 앞은 비워 둬요'); // (3,2)가 문
    expect(canPlace(s, 'kids_corner', X(5), Y(1)).reason).toBe('실내 가구는 건물 안에만 놓아요'); // (6,·)는 밖
  });
});
...
describe('손님: 겨울·비·태풍 실내 우선, 체류 시간, 둘러보기 (§8.1·P1-12)', () => {
  test('겨울엔 길이 이어진 실내 좌석이 있으면 실내를 먼저 고른다 (실내가 차면 야외)', () => {
    const s = cafe();
    placeObject(s, 'table_in', X(4), Y(1)); // 2석
  ...
  test('체류 시간 = 좌석 기본 + 순회 시설당 +8분(상한 6), 소파 +20%, 책장 +15%, 따뜻한 조명 저녁 +10%; 둘러보기 확률은 시설 3개 초과분 +5%', () => {
    ...
    placeObject(s, 'vending', X(4), Y(1)); // 이용료 시설 1
...
describe('실내 요소 상호작용 (§4.3)', () => {
  test('난로: 켜기/끄기, 12~2월 자동 ON, 반경 2 실내 좌석 겨울 만족 +3, 켜 두면 월 연료 ₩5만', () => {
    const s = cafe();
    const fire = placeObject(s, 'fireplace', X(4), Y(1));
    ...
    const s2 = cafe();
    const fire2 = placeObject(s2, 'fireplace', X(4), Y(1));
  ...
  test('소파 만족 +2, 바 저녁 세트 18시 이후 1인 손님 요금 +15%, 책장 신간(마일리지 1·한 달), 수족관 먹이(하루 1회·청결 +2), 키즈 장난감(₩10만·한 달·가족 +25%)', () => {
    const s = cafe();
    const sofa = placeObject(s, 'sofa_seat', X(4), Y(1));
