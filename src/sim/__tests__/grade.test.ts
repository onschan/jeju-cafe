...
/** 테마 1개(꽃길) — 트랙 C corners.json: 꽃밭 + 벤치 + 가로등 반경 2 */
...
/** 등급 g 조건을 딱 맞게 채운다 (테마는 활성 콤보 — 감귤나무+돌담 「돌담 수확」류가 없어도 되게 cornerCount를 넘기지 않고 조건에서 0으로 둔다) */
...
test('새 게임은 등급 1, 진행 4줄, 조건이 하나라도 모자라면 승급하지 않는다', () => {
  ...
  expect(gradeMet(s, 2)).toBe(false); // 테마 0/1
...
test('조건을 다 채우면 하루 한 단계 승급: 등급·알림·장면·박수 fx·보상 상자(응모권), rank.ts 훅으로도 돈다', () => {
  const s = bareState(1);
  meet(s, 2);
  // 테마 1: 꽃길 (트랙 C completedCorners — 꽃밭+벤치+가로등)
  flowerPath(s);
...
test('목표 사다리 새 조건: 등급·테마·단골·2층·평판·전설 직원·경로 수 판정과 문구', () => {
  const s = bareState(1);
  s.grade = 3;
  expect(conditionProgress(s, { type: 'grade', n: 4 })).toEqual({ cur: 3, max: 4 });
  expect(conditionProgress(s, { type: 'corners', n: 5 }).cur).toBe(0); // 목표 corners는 테마 도감(codex.corners, 트랙 C)
  ...
  expect(goalConditionText({ type: 'corners', n: 5 })).toBe('테마 5개');
