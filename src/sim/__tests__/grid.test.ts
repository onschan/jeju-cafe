...
test('빈 흙 칸에 밭을 놓을 수 있다', () => {
  ...
  expect(canPlace(s, 'field', X(0), Y(0)).ok).toBe(true);
  const obj = mustPlace(s, 'field', X(0), Y(0));
...
test('지형이 맞지 않으면 실패', () => {
  const s = createInitialState(1);
  expect(canPlace(s, 'field', X(0), Y(7)).ok).toBe(false); // 도로
  expect(canPlace(s, 'field', X(3), Y(0)).ok).toBe(false); // (3+0)%7===3 → rock
});

test('겹치거나 격자 밖이면 실패', () => {
  const s = createInitialState(1);
  expect(canPlace(s, 'field', X(3), Y(1)).ok).toBe(false); // 폐창고 위
  expect(canPlace(s, 'field', X(9), Y(9)).ok).toBe(false);
  expect(canPlace(s, 'field', -1, 0).ok).toBe(false);
});

test('다중 칸 오브젝트는 모든 칸을 검사한다', () => {
  const s = createInitialState(1);
  placeObject(s, 'field', X(1), Y(0));
...
test('제거하면 칸이 비고 객체가 사라진다', () => {
  const s = createInitialState(1);
  const obj = placeObject(s, 'field', X(0), Y(0));
...
test('북서쪽 돌담이 방풍을 만든다', () => {
  ...
  mustPlace(s, 'field', X(6), Y(6));
  expect(windShelter(s, X(6), Y(6))).toBe(0);
  expect(isSheltered(s, X(6), Y(6))).toBe(false);
  mustPlace(s, 'stonewall', X(5), Y(5)); // wind 2
  expect(windShelter(s, X(6), Y(6))).toBe(2);
  mustPlace(s, 'stonewall', X(4), Y(4)); // +2 → 4
  expect(isSheltered(s, X(6), Y(6))).toBe(true);
});
...
test('경치 점수 = 반경 2칸 풍경 합 − 소음 합', () => {
  ...
  mustPlace(s, 'tangerine_tree', X(7), Y(4)); // scenery 1
  mustPlace(s, 'stonewall', X(9), Y(5));      // scenery 1
  expect(sceneryScore(s, X(8), Y(4))).toBe(2);
...
test('소음이 경치를 깎는다', () => {
  const s = createInitialState(1);
  const seat = mustPlace(s, 'table_out', X(1), Y(6)); // 정류장 (0,7) noise 1, 반경 안
...
test('방풍 띠 밖(|dx−dy|>1)은 세지 않고, 여러 칸 오브젝트는 한 번만 센다', () => {
  ...
  mustPlace(s, 'stonewall', X(5), Y(3));
  expect(windShelter(s, X(6), Y(6))).toBe(0);
  // 창고 (3..5,1..2) wind 1: (5,4)의 띠에 (4,2)와 (3,2)가 모두 들어가지만 한 번만 → 1
  expect(windShelter(s, X(5), Y(4))).toBe(1);
});
