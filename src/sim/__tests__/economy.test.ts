...
import { ingredientCost, upkeep, closeMonth } from '../economy.ts';
...
test('창고 재료를 먼저 쓴다: 창고에 있으면 재료비 0·절감액 기록, 비면 원가로 자동 구매', () => {
  ...
  expect(ingredientCost(s, 'carrot_juice')).toBe(400);   // 정가 (창고 무시)
  expect(purchaseCost(s, 'carrot_juice')).toBe(400);     // 창고 없음 → 다 산다
  s.storage['carrot'] = 1;
  expect(ingredientCost(s, 'carrot_juice')).toBe(400);
  ...
  expect(s.monthHarvest.ingredientSaved).toBe(400);
  // 창고가 비면 자동 구매: 돈·이달 재료비가 빠지고 절감액은 그대로
  expect(consumeIngredients(s, 'carrot_juice')).toBe(400);
  expect(s.money).toBe(m0 - 400);
  expect(s.monthCosts.ingredients).toBe(400);
  expect(s.monthHarvest.ingredientSaved).toBe(400);
});
...
test('일부만 창고에 있으면 모자란 만큼만 산다 (당근케이크: 당근 2·밀가루·달걀)', () => {
  const s = bareState(1);
  s.storage['carrot'] = 1;
  expect(ingredientCost(s, 'carrot_cake')).toBe(400 * 2 + 600 + 600); // 정가 2000
  expect(purchaseCost(s, 'carrot_cake')).toBe(400 + 600 + 600);       // 창고 당근 1 → 1600
  const m0 = s.money;
  expect(consumeIngredients(s, 'carrot_cake')).toBe(1600);
  expect(s.money).toBe(m0 - 1600);
  expect(s.storage['carrot']).toBeUndefined();
  expect(s.monthCosts.ingredients).toBe(1600);
  expect(s.monthHarvest.ingredientSaved).toBe(400);
});
...
test('농원 월 수확: 지난달에 놓은 감귤나무가 1일에 창고를 채우고, 수확·절감액이 월말 카드에 남는다', () => {
  ...
  expect(s.monthHarvest.ingredientSaved).toBe(700);
  closeMonth(s, s.clock.month, s.clock.year);
  expect(s.lastMonthCard).toMatchObject({ harvested: { tangerine: 6 }, ingredientSaved: 700, topMenu: null });
