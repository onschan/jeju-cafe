...
test('초기 상태: 1년 3월 1일 6시, 36×30 격자(필지 9장, 12×10), 시작 오브젝트는 가운데 필지에', () => {
  const s = bareState(1);
  ...
  expect(s.grid.w).toBe(36);
  expect(s.grid.h).toBe(30);
  expect(s.grid.cells.length).toBe(1080);
  expect(START_ORIGIN).toEqual({ x: 12, y: 10 });
  // 시작 필지(12..23, 10..19)의 시작 오브젝트는 정류장·본관뿐 (bareState의 어귀 올렛길 토막은 빼고; 정낭은 없다 — zero-base)
  const inStart = (o: { x: number; y: number }) => o.x >= 12 && o.x < 24 && o.y >= 10 && o.y < 20;
  const types = Object.values(s.objects).filter(inStart).filter((o) => o.type !== 'path').map((o) => o.type).sort();
  expect(types).toEqual(['busstop', 'warehouse']);
  const bus = Object.values(s.objects).find((o) => o.type === 'busstop')!;
  expect([bus.x, bus.y]).toEqual([12, 19]); // 가운데 필지 남쪽 변 마을 길
...
test('마을 길(시작 필지 아래 변 y=19)은 맵 가로 전체 도로, 해안(오른쪽 아래)은 먼 변 2줄, 나머지는 전부 흙 (ease: 바위 없음)', () => {
  const s = bareState(1);
  expect(VILLAGE_ROAD_Y).toBe(19);
  for (let x = 0; x < GRID_W; x++) expect(s.grid.cells[VILLAGE_ROAD_Y * GRID_W + x]!.terrain).toBe('road');
  for (let x = 24; x < 36; x++) for (const y of [28, 29]) expect(s.grid.cells[y * GRID_W + x]!.terrain).toBe('road');
