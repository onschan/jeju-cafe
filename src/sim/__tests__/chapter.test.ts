...
import {
  ...
  countsForChapter, checkChapter, chapterLine, chapterResultLine,
} from '../chapter.ts';
import { rushState, rushSeatFits, rushGrades, RUSH_FIT_RADIUS } from '../rush.ts';
...
describe('막 (chapter)', () => {
  ...
  it('등급만 좋아도, 수만 채워도 못 넘는다 — 둘 다 같은 판에서', () => {
    const s = bareState(1);
    const need = CHAPTERS[0]!.need;
    expect(checkChapter(s, rushWith(s, need), 'B')).toBe(false);      // 등급 모자람
    expect(checkChapter(s, rushWith(s, need - 1), 'A')).toBe(false);  // 수 모자람
    expect(chapterProgress(s).idx).toBe(0);
    const tickets = s.tickets;
    expect(checkChapter(s, rushWith(s, need), 'A')).toBe(true);
  ...
  it('5막은 손님층이 아니라 S 한 판 — 다 넘기면 막이 끝난다', () => {
    const s = bareState(1);
    for (let i = 0; i < 4; i++) expect(checkChapter(s, rushWith(s, CHAPTERS[i]!.need), 'A')).toBe(true);
    const last = currentChapter(s)!;
    expect(last.want).toBeNull();
    expect(checkChapter(s, rushWith(s, 0), 'A')).toBe(false); // A로는 못 넘는다
    expect(checkChapter(s, rushWith(s, 0), 'S')).toBe(true);
  ...
  it('결과 카드 한 줄: 못 넘었으면 무엇이 모자랐나, 넘었으면 그 막의 끝말', () => {
    ...
    expect(chapterResultLine(s, r, 'A', false)).toContain('1/2');
    expect(chapterResultLine(s, rushWith(s, 2), 'B', false)).toContain('A');
    checkChapter(s, rushWith(s, 2), 'A');
