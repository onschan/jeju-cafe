...
/** 그 직원의 다음 연수 비용 (n번째 = trainingCount + 1) */
export function trainingCost(staff: { trainingCount: number }, trainingId: string): number {
  return Math.round(trainingDef(trainingId).cost * (1 + TRAINING_COST_STEP * staff.trainingCount));
}
...
export function canTrain(state: GameState, staffId: string, trainingId: string): ApplyResult {
  ...
  if (state.money < trainingCost(st, trainingId)) return { ok: false, reason: '돈이 모자라요' };
  return { ok: true };
}
...
export function train(state: GameState, staffId: string, trainingId: string): void {
  const st = findStaff(state, staffId)!;
  const cost = trainingCost(st, trainingId);
  state.money -= cost;
...
export function trainingOptions(state: GameState, staffId: string): { def: TrainingDef; cost: number; ok: boolean; reason?: string }[] {
  ...
  return TRAININGS.map((def) => {
    ...
    return { def, cost: st ? trainingCost(st, def.id) : def.cost, ok: r.ok, reason: r.reason };
  });
}
