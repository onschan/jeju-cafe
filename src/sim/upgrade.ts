...
export function canUpgrade(state: GameState, objId: string, popularity: number): ApplyResult {
  ...
  if (next === 2 && uses < UPGRADE_USES[2]! && popularity < UPGRADE_POP_ALT) return { ok: false, reason: `이용 ${UPGRADE_USES[2]}회 또는 인기 ${josa(String(UPGRADE_POP_ALT), '이/가')} 필요해요 (지금 ${uses}회 · ${popularity})` };
  if (next === 3) {
