...
import { completedCorners, cornerTags } from './corners.ts'; // 트랙 C 테마 판정
...
/** 요청을 들어줬나: 메뉴 → 메뉴판에 있다, 테마 → 트랙 C completedCorners(테마가 있으면 테마 우선, 없으면 대체 시설), 시설 → 소유 필지에 완공 */
...
export function requestHint(def: GuestRequestDef): string {
  ...
  if (w.corner || w.tagCorner) return '짓기 창 테마 탭에 있어요';
