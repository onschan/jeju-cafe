/**
 * 시설 능력치 합산 (trim: 콤보 12·명당 12를 걷어내고 코너 24 + 세트만 남겼다).
 *
 * 인기 = 기본 + 강화 아이템 + 손님 효과 + 코너 + 세트 배수 → 상한 40 → 증축 Lv 가산 − 노후.
 * 요금% = 기본 + 강화 아이템 + 증축 Lv + 명소 Lv + 코너.
 */
...
/** 손님층이 이 시설을 고를 확률 배수: 반경 안 코너 중 태그가 맞는 것마다 ×tagMult, 최대 ×2.0. — guests.ts(E) pickVisit 훅용 */
...
/** 코너 만족 가산 (태그가 맞으면 +5, 전체 대상 코너는 +3). — guests.ts(E) 만족 판정 훅용 */
...
/** 아이템·손님 효과(시설 인기)·코너를 더한 인기(세트 배수 전)와 요금 %(Lv +10%/+20%, 좌석형은 +5%/+10%, 명소 Lv +2%/+5%) */
function rawStats(state: GameState, obj: PlacedObject): { pop: number; feePct: number } {
  ...
  const corner = cornerBonusAt(state, obj); // 반경 안 완성 코너 인기 +5·요금 +5% (합산 상한)
...
/** 기본(ObjectDef) + 아이템 + 코너 + 전체 대상 세트 배수 + 계절 경치. 손님층별 값은 popularityFor. */
...
/** 손님층이 보는 인기 = (기본 + 아이템 + 코너) × 그 태그에 맞는 세트 배수, 상한 40 */
...
/** 배치·이동·완공 뒤: 처음 완성한 세트·코너를 도감에 올린다. */
