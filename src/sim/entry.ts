...
export function cruiseDocked(state: GameState): boolean {
  ...
  return state.events.some((e) => e.id === CRUISE_EVENT && e.startDay <= today && e.endsDay > today);
}
...
export function chargePortFee(state: GameState): void {
  ...
  const today = dayIndex(state.clock);
  const ev = state.events.find((e) => e.id === CRUISE_EVENT && e.startDay <= today && e.endsDay > today);
