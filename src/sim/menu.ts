...
import { menuDef, ingredientDef, roleDef, skillDef } from '../data/index.ts';
...
/** 메뉴의 직원 조건(역할 또는 스킬)을 배치된 직원(기력 > 0) 중 누군가 만족하나. 조건 없으면 true(주인이 만든다). */
export function hasMenuStaff(state: GameState, menuId: string): boolean {
  const req = menuDef(menuId).requires;
  if (!req) return true;
  return state.staff.some((st) => st.role !== null && st.energy > 0 && (req.role === undefined || st.role === req.role) && (req.skill === undefined || st.skill === req.skill));
}

/** UI용: "바리스타 필요" 같은 조건 문구. 조건 없으면 null. */
export function menuRequirementText(menuId: string): string | null {
  const req = menuDef(menuId).requires;
  if (!req) return null;
  const parts: string[] = [];
