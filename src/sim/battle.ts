...
/** 러시는 매주 토요일 — 1~5일 준비, 6·7일이 주말이라 「토요일」은 day % 7 === 6.
 *  rush1이 러시 요일을 잡을 때 이 값을 그대로 쓴다 (TODO(rush1): rush.ts에서 import). */
...
export function battleDay(): number {
  for (let d = DAYS_PER_MONTH; d >= 1; d--) if (d % 7 === RUSH_WEEKDAY) return d;
...
export function dailyBattle(state: GameState): void {
  ...
  startBattle(state);
  // TODO(rush1): 러시가 붙으면 러시 끝에서 resolveBattle(state, 러시 점수)를 부른다.
  // 러시가 아직 없거나 봇·헤드리스면 여기서 자동 해결한다 — 결정적이라 KPI 밴드가 흔들리지 않는다.
  if (!rushWillResolve(state)) resolveBattle(state);
}
/** 러시가 이 판을 직접 끝낼 것인가.
 *  TODO(rush1): rush.ts가 생기면 `rushEnabled(state)`로 바꾼다. */
function rushWillResolve(state: GameState): boolean {
  return (state as unknown as { rush?: { enabled?: boolean } }).rush?.enabled === true;
}
