...
export function targetMult(state: GameState, typeId: string): number {
  return state.targetSegment === typeId ? TARGET_MULT : 1;
}
...
export function effectivePopularity(state: GameState, typeId: string): number {
  return (state.segmentPopularity[typeId] ?? 0) + activeBonus(state, typeId);
}

export function youtuberMultiplier(state: GameState, typeId: string): number {
  return state.youtuberBoostMonths > 0 && typeId === 'tourist' ? YOUTUBER_TOURIST_MULT : 1;
}
...
export function canSetTarget(state: GameState, segment: string | null): ApplyResult {
  if (segment !== null && !GUEST_TYPES.some((t) => t.id === segment)) return { ok: false, reason: '모르는 손님층이에요' };
  return { ok: true };
}
