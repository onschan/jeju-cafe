...
/** 소유 필지에 놓인 오브젝트의 월 유지비 합을 차감한다 (아직 안 산 필지의 돌담 등은 제외). 월말에 closeMonth보다 먼저 호출한다. */
export function upkeep(state: GameState): void {
  let sum = 0;
  for (const obj of Object.values(state.objects)) if (parcelAt(state, obj.x, obj.y)?.owned) sum += objectDef(obj.type).upkeep;
  state.money -= sum;
