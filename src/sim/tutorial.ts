/**
 * 손으로 하는 튜토리얼 「할망의 가르침」 33단계 · 5장 (§7.2 확장 + w-start 맨땅 시작 + pro-guide 공략 톤). 1년차 전체를 관통하며 모든 시스템을 한 번씩 직접 해 본다.
 * w-start: 새 게임은 맨땅(본관 없음)에서 시작한다. 1~3단계(둘러보기·본관 짓기·본관 보기)를 앞에 넣어 기존 30단계는 번호가 3씩 밀렸다.
 * pro-guide: 화자는 전부 할망 — 최적 수순을 제주 말투로 안내한다. 글로우 칸은 고정 좌표가 아니라 strategy.ts가 실제 입지·콤보 수치로 고른 최적 칸
