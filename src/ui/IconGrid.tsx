...
export function IconGrid<K extends string>({ items, active, onPick, cols = 2, testId, tutPrefix = 'tab' }: {
  items: IconGridItem<K>[]; active?: K | null; onPick: (k: K) => void; cols?: number; testId?: string; tutPrefix?: string;
}) {
  return (
    <div data-testid={testId} role="tablist" style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))`, gap: 6, marginBottom: 8 }}>
      {items.map((it) => {
