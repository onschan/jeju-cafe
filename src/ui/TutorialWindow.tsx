/** 튜토리얼 창 「할망의 가르침」: 「진행」 탭 = 7단계 목록(완료 ✓ · 지금 ▶ · 아직 ○)과 현재 단계 카드(대사 첫 줄·할 일), 대사 다시 보기, 남은 단계 건너뛰기(보상 없음), 스포트라이트 켜기/끄기.
 *  「추천」 탭 = 할망의 추천 = solver(sim/solver.ts bestMoves, 워커) 상위 3수 + 각 예상 이득("14일 뒤 자금 +₩42만") — 탭하면 그 행동의 타깃(칸·창 버튼)이 글로우(tutorialHighlight setGuideFocus).
 *  결과가 아직 없으면 다음 수 한 줄(strategy.heuristicNextMove) + 「셈하는 중…」. 그 아래 1년차 월별 표(strategy.openingBuild). 3일 무행동 힌트(hints.ts)도 같은 1위 수.
 *  목표 줄 왼쪽 「📖 n/7」 배지에서 연다 (GoalBar.tsx). 진행은 sim 상태(state.tutorial)에서 읽는다. 장(章)은 없다(fun-start). */
import { useState } from 'react';
import { useGame } from './store';
import { Popup, confirm } from './Popup';
import { PALETTE, brownBtn, brownBtnOn, card } from './frame';
import { TUTORIAL_STEPS, currentTutorialStep, tutorialDone, tutorialStepDone, openingBuild, heuristicNextMove, solverResult, solverKey, UI_SOLVER_OPTIONS } from '../sim/index.ts';
...
export const PLAN_TITLE = '1년차 이렇게 해 보라';

/** 추천 탭: 할망의 추천(solver 상위 3 + 예상 이득, 탭하면 글로우) + 월별 표 */
export function StrategyNote({ onFocus }: { onFocus?: () => void } = {}) {
  ...
  const focus = (i: number) => {
    const m = moves[i]!;
    setGuideFocus({ key: solverKey(s), targets: m.targets, cells: m.cells, label: `${m.label} · ${m.why}` });
    onFocus?.();
  };
  return (
    <div data-testid="tutorial-note">
      <div data-testid="tutorial-next-move" style={{ ...card, marginBottom: 8, borderColor: PALETTE.title }}>
        <div style={{ fontWeight: 700 }}>🎯 {RECOMMEND_TITLE} ({res ? res.horizon : UI_SOLVER_OPTIONS.horizon}일 뒤를 본 것) <span style={{ fontWeight: 400, color: PALETTE.inkSoft, fontSize: 13 }}>{res ? '' : solverBusy() ? SOLVER_BUSY_TEXT : ''}</span></div>
      ...
      <div style={{ fontWeight: 700, marginBottom: 4 }}>📅 {PLAN_TITLE}</div>
...
export function TutorialWindow({ onClose }: { onClose: () => void }) {
  ...
  const skip = () => {
    void confirm(SKIP_TEXT, { title: '남은 가르침 건너뛰기', yes: '건너뛰기', no: '계속 배우기' }).then((ok) => { if (ok) { skipCurrentChapter(); onClose(); } });
  };
  const openNote = () => { setTab('note'); showFirstTip('recommend'); };
  return (
    <Popup title={`📖 할망의 가르침 ${Math.min(s.tutorial.step, TUTORIAL_STEPS)}/${TUTORIAL_STEPS}`} onBackdrop={onClose}
      buttons={<>
        {!done && <button data-testid="tutorial-skip-chapter" style={{ ...brownBtn, margin: 0, whiteSpace: 'nowrap' }} onClick={skip} aria-label="남은 가르침 건너뛰기">⏭ 건너뛰기</button>}
      ...
      <div data-testid="tutorial-window" style={{ fontSize: 14 }}>
        <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
          ...
          <button data-testid="tutorial-tab-note" aria-pressed={tab === 'note'} style={{ ...(tab === 'note' ? brownBtnOn : brownBtn), margin: 0, flex: 1, minHeight: 44, fontSize: 14 }} onClick={openNote}>📖 {NOTE_TAB}</button>
        </div>
        ...
        {tab === 'progress' && (
          <ul data-testid="tutorial-steps" style={{ margin: '2px 0 0 4px', padding: 0, listStyle: 'none', fontSize: 14, lineHeight: 1.7 }}>
            {TUTORIAL_DIALOGUES.map((d) => {
              ...
              return (
                <li key={d.id} data-testid={`tutorial-step-${d.id}`} style={{ color: isDone ? PALETTE.inkSoft : isCur ? PALETTE.title : PALETTE.ink, fontWeight: isCur ? 700 : 400 }}>
                  {isDone ? '✓' : isCur ? '▶' : '○'} {d.id}. {fillTutorialStep(d, s).title}
                </li>
              );
            })}
          </ul>
