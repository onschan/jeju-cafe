  ...
  return (
    <div data-testid="card-route" data-route={route}>
      <div style={{ fontSize: 14, lineHeight: 1.5 }}>
        <div><span aria-hidden>{def.icon}</span> <b>{def.name}</b> <span style={small}>{facility && facility.type !== 'busstop' ? objectDef(facility.type).name : ''}{parcel ? ` · ${parcel.name}${parcel.owned ? '' : ' 필요'}` : ''}</span></div>
        <div style={small}>오늘 손님 <b style={{ color: PALETTE.ink }}>{st.todayGuests}</b>명 · 이달 {st.monthGuests}명 · 다음 도착 {route === 'bus' ? `상시 · 투어 버스 ${nextTourBusText(s)}` : nextArrivalText(s, route)}</div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>
        {def.needsContract && (st.contract
          ...
          : <button style={contractOn.ok ? btnOn : btnOff} disabled={!contractOn.ok} title={contractOn.ok ? undefined : contractOn.reason} onClick={contract} data-testid="route-contract">✈️ 계약 ({won(SHUTTLE_FEE)}/월)</button>)}
        {route === 'parking' && lot && <button style={expand.ok ? btnOn : btnOff} disabled={!expand.ok} title={expand.ok ? undefined : expand.reason} onClick={doExpand} data-testid="route-expand">🚗 넓히기 ({won(parkingExpandCost())})</button>}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '6px 8px', fontSize: 14, alignItems: 'center' }}>
        {rows.map((r) => (
          <span key={r.route} style={{ display: 'contents' }}>
            <span style={{ color: r.unlocked ? PALETTE.ink : PALETTE.inkSoft, whiteSpace: 'nowrap' }}><span aria-hidden>{r.icon}</span> {r.name}{!r.unlocked ? ' (잠김)' : !r.active ? ' (멈춤)' : ''}</span>
