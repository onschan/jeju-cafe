...
import { computeScore, makeCarry, carryText } from '../sim/index.ts';
...
export function drawEnding(canvas: HTMLCanvasElement, sheet: Sheet | null, s: GameState) {
  ...
  drawFrame(ctx, sheet, sheet.frames[main] ? main : 'iso_obj_warehouse', SCENE_W / 2, oy + 34, { anchorX: 0.5, anchorY: 1 });
  // 직원 줄지어 (없으면 앞치마 인물 1)
...
function EndingCanvas({ s }: { s: GameState }) {
  ...
  useEffect(() => {
    ...
    if (!c) return;
    drawEnding(c, null, s);
    void loadSheet().then((sheet) => { if (!disposed && ref.current) drawEnding(ref.current, sheet, s); });
...
export function EndingScreen({ onExit }: { onExit: () => void }) {
  const s = useGame();
  const a = s.alerts[0];
  const show = a?.type === 'ending' && !s.lastMonthCard && !s.lastAnnouncement; // 결산 카드·가이드북 발표 뒤에
  const [line, setLine] = useState(0);
  const [phase, setPhase] = useState<'scene' | 'score'>('scene');
  const [record, setRecord] = useState(false);
  const score = useMemo(() => s.ending.score ?? computeScore(s), [s.ending.score]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => { if (show) return pauseGame('dialogue'); }, [show]);
  useEffect(() => { if (show) { setLine(0); setPhase('scene'); sfx('fanfare'); } }, [show]);
  useEffect(() => { if (phase === 'score' && score) setRecord(recordEnding(score)); }, [phase, score]);
  if (!show) return null;
  const lines: SpokenLine[] = ENDING_DIALOGUES.ending.lines;
  const title = ENDING_DIALOGUES.ending.title;
  const next = () => {
      ...
      <div style={{ ...frame, width: '100%', maxWidth: 360, maxHeight: '100%', overflowY: 'auto' }}>
        ...
        {phase === 'scene' ? (
          <div onClick={next} data-testid="ending-cut">
            ...
            <button style={{ ...fullBtn, marginTop: 8 }} onClick={(e) => { e.stopPropagation(); next(); }} aria-label={line + 1 < lines.length ? '다음' : '점수 보기'}>
              {line + 1 < lines.length ? '▶ 다음' : '📜 점수 보기'}
            </button>
          </div>
        ) : score && (
          <div>
