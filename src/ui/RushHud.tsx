...
import {
  RUSH_LEN_MS, fireSkill, noteRushSkill, rushOf, rushPaused, rushRunning, rushSkillOf, rushSkillStaff,
  rushTick, rushTimeLeftMs, skillCooldownMs, subscribeRush, useRushAutoPref, type RushQueueGuest,
} from './rushBridge';
...
export function RushHud() {
  ...
  // 임시 엔진을 굴리는 rAF. 멈춰 있으면 dt=0으로 넣어 시간이 흐르지 않게 한다.
  useEffect(() => {
    if (!running) return;
    let raf = 0;
  ...
  const staff = rushSkillStaff(s);
  const pickFront = () => showMessage('초록 자리를 눌러 앉혀요');
  const press = (staffId: string) => {
    const out = fireSkill(s, staffId);
  ...
  return (
    <>
      ...
      <div data-testid="rush-cards" role="toolbar" aria-label="직원 스킬" style={{
        ...
        {staff.map((w) => {
          const def = rushSkillOf(w.role)!;
          const cd = skillCooldownMs(s, w.id);
          const ratio = cd / def.cooldownMs;
          return (
            <button key={w.id} data-testid={`rush-skill-${w.id}`} data-tut="rush-skill" onClick={() => press(w.id)} disabled={ratio > 0}
              aria-label={`${w.name}의 ${def.name}${ratio > 0 ? ` · ${Math.ceil(cd / 1000)}초 뒤` : ''}`} title={def.text}
              style={{
              ...
              <span style={{ fontSize: 11, color: PALETTE.inkSoft, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%' }}>{w.name}</span>
              <Cooldown ratio={ratio} />
              {ratio > 0 && <span style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff8e6', fontSize: 16 }}>{Math.ceil(cd / 1000)}</span>}
            </button>
          );
        })}
      </div>
