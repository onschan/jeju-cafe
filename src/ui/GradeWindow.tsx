 ...
 *  아래: 다음 등급 조건 4줄(다녀간 손님·완성한 테마·평판·★) 진행 막대 + 등급이 오르면 생기는 것. */
  ...
  return (
    <Window title={`등급 「${gradeName(cur)}」`} onClose={onClose} testId="window-grade">
      ...
      <div style={{ fontSize: 13, color: PALETTE.inkSoft, marginTop: 8, lineHeight: 1.4 }}>
        등급 조건: {Object.entries(GRADE_REQS).map(([g, r]) => `${gradeName(Number(g))} 손님 ${fmtNum(r.guests)}·테마 ${r.corners}·평판 ${r.reputation}·★${r.star}`).join(' / ')}
      </div>
    </Window>
  );
}
