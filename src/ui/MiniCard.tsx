  ...
  return (
    <div data-testid="card-guest">
      ...
      <Row>
        ...
        {giftCount(s) > 0 && <button data-tut="gift" style={giftOk ? btn : btnOff} disabled={!giftOk} onClick={() => setPicking(true)} aria-label="선물하기">🎁 선물하기{giftedToday(s) ? ' (내일)' : ''}</button>}
      </Row>
      {picking && (
...
export function MainCard({ s, id, a }: { s: GameState; id: string; a: CardActions }) {
  ...
  return (
    <div data-testid="card-main">
      ...
      <Row>
        {next && <button style={exp.ok ? mbtnOn : mbtnOff} disabled={!exp.ok} title={exp.ok ? undefined : exp.reason} onClick={doExpand} data-testid="main-expand-btn" data-tut="main-expand">🔨 증축 Lv{next} ({manWon(expandCost(s))}·{MAIN_EXPAND_DAYS}일)</button>}
